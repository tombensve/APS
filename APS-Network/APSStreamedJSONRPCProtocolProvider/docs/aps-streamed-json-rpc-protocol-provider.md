...
JSONREST version 1.0 extending JSONHTTP and providing 'true' for _supportsREST()_ which will make the http transport always map methods annotated with @APSRemoteService(httpMethod=GET/PUT/POST/DELETE). This can thereby deliver a reasonable REST API. Do note that for the other protocols the service itself does not need to be designed for remote availability, but for REST that is required by using the @APSRemoteService annotation. You cannot just take any service an make it available as a REST service!
