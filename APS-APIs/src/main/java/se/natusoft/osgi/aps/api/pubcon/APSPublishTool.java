 ...
 */
public class APSPublishTool implements APSPublisher {
     ...
     *
     * @param o    The published data.
     * @param meta Meta data about the published data.
     */
    @SuppressWarnings("unchecked")
    @Override
    public void apsPublish(Object o, Properties meta) {
        if (this.mode == Mode.CURRENT_AND_FUTURE) {
            this.tracker.onServiceAvailable((service, serviceRef) -> ((APSConsumer)service).apsConsume(o, meta));
        }
        else {
            this.tracker.withAllAvailableServices((service, args) -> ((APSConsumer) service).apsConsume(o, meta));
        }
    }
